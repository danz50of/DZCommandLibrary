CREATE OR REPLACE FUNCTION Get_Labor_Setup_Factor (part_no_ in varchar2, contract_ in varchar2,
op_no_ in varchar2, order_no_ in varchar2, account_date_ in date)
return number
IS
standard number(9,4);
CURSOR Get_labor_factor IS
select labor_setup_time from ifsapp.routing_operation c where c.part_no = part_no_
...
grant execute on IFSAPP.Get_Total_Labor_Duration to IFSINFO;
