-- FLIP BACKORDER LOGIC TO BE QTY_AVAILABLE - OPEN CUSTOMER PO.  RECEIVING 0 IN ONHAND AVERAGE CALCULATION
...
((QTY_AVAILABLE - (QTY_AVAILABLE - OPEN_CUSTOMER_ORDER)) / (AVRG_USAGE + .0000000000001)) MO_ONHAND_AVERAGE,
((QTY_AVAILABLE - (QTY_AVAILABLE - OPEN_CUSTOMER_ORDER) + OPEN_SUPPLIER_QTY) / (AVRG_USAGE + .000000000001))MO_ONHAND_WITH_CURRENT_ORDERS
FROM
(select a.part_no,
       b.description Descr,
      ...
      where   
         ...
         c.contract(+) and
         a.part_no between nvl(upper('&BEG_PART'),'                  ') and 
          nvl(upper('&END_PART'),'ZZZZZZZZZZZZZZZZZZZZ')
         and upper(a.contract) = nvl(upper('&SITE'),'MP') and 
         b.accounting_group between nvl(upper('&BEG_ACCT_GRP'),'            ') and 
         nvl(upper('&END_ACCT_GRP'),'ZZZZZZZZZZZZZZZ'))