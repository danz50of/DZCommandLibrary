...
WHERE A.ORDER_NO = B.ORDER_NO and B.CUSTOMER_NO = c.customer_no and ((trunc(a.date_entered,'yy')>=trunc(sysdate,'yy')) AND (trunc(a.date_entered,'dd')<=trunc(sysdate,'dd')) AND (A.CONTRACT='MP') OR (trunc(a.date_entered,'yy')>=trunc(add_months(sysdate,-12),'yy')) AND (trunc(a.date_entered,'dd')<=trunc(add_months(sysdate,-12),'dd')) AND (A.CONTRACT='MP'))
